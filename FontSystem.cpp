...
namespace libre::ui {
    ...
    bool FontSystem::loadFontFamily(const std::string& familyName,
        ...
        if (!boldPath.empty()) {
            ok &= loadFont(familyName, boldPath, FontWeight::Bold);
        }
        if (!lightPath.empty()) {
            ok &= loadFont(familyName, lightPath, FontWeight::Light);
        }
        return ok;
    }
    ...
    void FontSystem::growAtlas() {
        ...
        // For simplicity, recreate atlas and re-rasterize all glyphs
        // A production system would copy existing data
    ...
    void FontSystem::uploadAtlasToGPU(VkCommandBuffer cmd) {
        ...
        // Staging cleanup will happen after command buffer execution
        // In production, use a deletion queue
    }
    ...
    void TextLayout::layout(const std::string& text, FontFace* font,
        ...
        if (options.maxWidth > 0) {
            lines = FontSystem::instance().wrapText(text, options.maxWidth, font);
        }
        else {
            // Split by newlines
            std::istringstream stream(text);
        ...
        // Apply max lines
        if (options.maxLines > 0 && lines.size() > static_cast<size_t>(options.maxLines)) {
            lines.resize(options.maxLines);
            if (options.ellipsis && !lines.empty()) {
                lines.back() += "...";
            }
        }
        ...
        for (const auto& line : lines) {
            float lineWidth = FontSystem::instance().measureText(line, font).x;
            float x = 0;

            // Apply horizontal alignment
            if (options.align == TextAlign::Center) {
                x = (options.maxWidth - lineWidth) / 2.0f;
            }
            else if (options.align == TextAlign::Right) {
                x = options.maxWidth - lineWidth;
            }
            ...
            for (char c : line) {
                ...
                LayoutGlyph lg;
                lg.glyph = const_cast<Glyph*>(&g);
                lg.position = glm::vec2(
                    x + g.bearing.x,
                    y - g.bearing.y
                );
                lg.color = color;

                glyphs_.push_back(lg);
                x += g.advance / 64.0f;
            }

            bounds_.x = std::max(bounds_.x, x);
            y += lineHeight;
        }

        bounds_.y = y - lineHeight + font->descender;
    }
